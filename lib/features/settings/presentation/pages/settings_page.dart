...
class SettingsPage extends ConsumerWidget {
          ...
          child: Column(
            ...
            children: [
              ...
              Container(
                ...
                child: ListTile(
                  leading: Icon(Icons.swap_horiz_rounded, color: AppColors.primaryGreen),
                  title: Text(
                    'Cambiar a ${targetProfile == Roles.chofer ? 'Conductor' : 'Pasajero'}',
                    style: const TextStyle(fontWeight: FontWeight.w600),
                  ),
                  trailing: const Icon(Icons.arrow_forward_ios, size: 16),
                  onTap: () => _showSwitchRoleDialog(context, ref, targetProfile),
                ),
              ),
              const SizedBox(height: 30),
              // Botón Cerrar Sesión
  ...
  void _showSwitchRoleDialog(BuildContext context, WidgetRef ref, String targetProfile) {
    final authState = ref.watch(authProvider);
    final currentUser = authState.value?.usuario;
    ...
    final humanTarget = targetProfile == Roles.chofer ? 'Conductor' : 'Pasajero';
    ...
    showDialog(
      ...
      builder: (BuildContext dialogContext) {
        return AlertDialog(
          ...
          content: Text('¿Seguro que quieres cambiar al perfil de $humanTarget?'),
          actions: [
            TextButton(
              child: const Text('Cancelar'),
              onPressed: () {
            ...
            ElevatedButton(
              ...
              onPressed: () async {
                context.pop();
                try {
                  await ref.read(authProvider.notifier).changeProfile(
                        currentUser.id,
                        targetProfile, 
                      );
                  ...
                  if (context.mounted) {
                    ...
                    // Navega según el perfil destino
                    if (targetProfile == Roles.chofer) {
